...
def set_auth_cookies(response: Response, raw: str, csrf_token: str):
    response.set_cookie("nexgene_session", raw, httponly=True, secure=COOKIE_SECURE, samesite="lax", max_age=SESSION_DAYS * 86400, path="/")
    response.set_cookie("nexgene_csrf", csrf_token, httponly=False, secure=COOKIE_SECURE, samesite="lax", max_age=SESSION_DAYS * 86400, path="/")
...
def rate_limit(request: Request, key: str, limit: int, s: Session):
    ...
    with _RATE_LOCK:
        return _rate_limit_locked(s, bucket_key, window_start, limit)

def _rate_limit_locked(s: Session, bucket_key: str, window_start: int, limit: int):
    row = s.scalar(select(RateLimitBucket).where(RateLimitBucket.bucket_key == bucket_key, RateLimitBucket.window_start == window_start))
    if row is None:
...
def csrf(request: Request, response: Response, s: Session = Depends(db)):
    ...
    if st:
        ...
        s.commit()
        response.set_cookie("nexgene_csrf", new_token, httponly=False, secure=COOKIE_SECURE, samesite="lax", path="/", max_age=SESSION_DAYS * 86400)
    return {"status": "ok"}
...
def verify_email(token: str, request: Request, response: Response, s: Session = Depends(db)):
    ...
    row = s.scalar(select(OneTimeToken).where(OneTimeToken.token_hash == hash_token(token), OneTimeToken.purpose == "verify", OneTimeToken.used_at.is_(None)))
...
def get_profile(u: User = Depends(current_user), s: Session = Depends(db)):
    ...
    data = {"age_range": p.age_range, "country": p.country, "occupation": p.occupation, "student": p.student, "study_field": p.study_field, "schedule": p.schedule, "timezone": p.timezone}
...
def save_checkin(period, data, u, s):
    ...
    for kind, value in data.values.items():
        s.add(Observation(user_id=u.id, kind=kind, value_numeric=float(value) if isinstance(value, (int, float)) else None, value_text=None if isinstance(value, (int, float)) else str(value), recorded_at=recorded))
    s.commit()
...
def weekly_report(u: User = Depends(current_user), s: Session = Depends(db)):
    ...
    rows = s.scalars(select(Observation).where(Observation.user_id == u.id, Observation.recorded_at >= baseline_start).order_by(Observation.recorded_at.asc())).all()
    week = [r for r in rows if as_utc(r.recorded_at) >= start]
    active_days = len({r.recorded_at.date().isoformat() for r in week})
    nums_week, nums_base = defaultdict(list), defaultdict(list)
    for r in week:
        if r.value_numeric is not None:
    ...
    sleep, sleep_base = avg(nums_week.get("sleep_duration", [])), avg(nums_base.get("sleep_duration", []))
    ...
    focus, stress, energy = avg(nums_week.get("focus", [])), avg(nums_week.get("stress", [])), avg(nums_week.get("energy", []))
    ...
    return {"status": "ready", "title": "Your week with NexGene", "subtitle": "A small, human read of the signal so far.", "coverage": coverage, "profile": context, "sections": sections[:4], "experiment": experiment, "disclaimer": "This report describes patterns in your recorded data. It is not a diagnosis or medical advice."}
...
def signals(u: User = Depends(current_user), s: Session = Depends(db)):
    ...
    if not rows:
        return {"status": "starting", "signals": [{"type": "welcome", "title": "Let's start with one small signal.", "body": "A morning or evening check-in gives NexGene something to learn from.", "action": "check_in"}]}
    by_day, numeric = defaultdict(list), defaultdict(list)
    for r in rows:
    ...
    signals_out = []
    if active_days < 3:
        signals_out.append({"type": "curiosity", "title": "Your pattern is just beginning.", "body": f"You've given NexGene {active_days} day{'s' if active_days != 1 else ''} of signal. A few more will make the picture more interesting.", "action": "check_in"})
    elif active_days < 7:
        remaining = 7 - active_days
        signals_out.append({"type": "milestone", "title": "The first pattern is coming into focus.", "body": f"You've recorded {active_days} days. {remaining} more day{'s' if remaining != 1 else ''} will give NexGene a fuller first-week window.", "action": "check_in"})
    else:
        signals_out.append({"type": "milestone", "title": "You've built a real thread.", "body": f"NexGene has {active_days} days of signal to work with. Now the interesting part is seeing what moves together.", "action": "patterns"})
    def delta_signal(kind, label, unit, threshold):
        ...
        if abs(delta) < threshold:
            return None
        direction = "up" if delta > 0 else "down"
        return {"type": "change", "title": f"Something changed in your {label}.", "body": f"Your latest readings are about {abs(delta):.1f} {unit} {direction} from the previous few. It may be worth watching what happens next.", "action": "patterns"}
    for args in [("sleep_duration", "sleep", "hours", 0.5), ("stress", "stress", "points", 1.0), ("focus", "focus", "points", 1.0), ("energy", "energy", "points", 1.0)]:
    ...
    if len(paired) >= 4:
        ...
        if high_sleep and low_sleep:
            hi, lo = sum(high_sleep) / len(high_sleep), sum(low_sleep) / len(low_sleep)
            if abs(hi - lo) >= 1.0:
                signals_out.append({"type": "relationship", "title": "There's a thread between sleep and focus.", "body": "On the days you've slept more, your focus readings have tended to move differently. That's a pattern to keep watching, not a conclusion.", "action": "patterns"})
...
def insights(u: User = Depends(current_user), s: Session = Depends(db)):
    ...
    def add_delta(kind, label, unit):
        v = nums.get(kind, [])
        if len(v) >= 6:
            recent, earlier = sum(v[:3]) / 3, sum(v[3:6]) / 3
